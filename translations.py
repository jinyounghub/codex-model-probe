...
EN = {
    ...
    "요청 모델·세션 ID는 실제 요청 메시지에서, 최종 모델은 서버의 완료 응답에서 읽습니다. 같은 응답으로 연결할 수 없으면 요청 정보는 '확인 불가'로 표시합니다. 기존 프록시를 감시하는 창에서는 원래 모니터를 켜두세요.":
        "Requested model and session ID come from the actual request; final model comes from the server's completed response. Unpaired requests are shown as Unknown. Keep the original monitor open when watching its proxy.",
