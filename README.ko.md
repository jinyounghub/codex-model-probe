...
실제 한글판 사용 화면입니다. **시각·프로젝트명·대화 제목과 ID는 공개용 이미지에서 익명화한 뒤 블러 처리했습니다.** 모델명·reasoning·토큰 수를 포함한 가림 영역 밖의 픽셀은 원본 그대로입니다. 프로그램 자체에는 확인 가능한 원래 값이 표시됩니다.
...
가로 스크롤로 오른쪽의 ID, 통신 방식, 최종 페이로드 근거 필드를 확인할 수 있습니다. 개인 식별 영역의 블러는 이 게시용 이미지에만 적용한 가림 처리입니다.
